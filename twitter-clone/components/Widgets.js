...
export default function Widgets({news, randomUser}) {
    const [newsNum, setNewsNum ] = useState(3)
    ...
    return (
        <div className="xl:w-[600px] hidden lg:inline ml-8 space-y-5">
            ...
            {/*news components start*/}
            <div className="text-gray-700 space-y-3 bg-gray-100 rounded-xl pt-2 w-[90%] xl:w-[75%]">
                <h4 className="font-bold text-xl px-4">What's happening</h4>
                {news.slice(0, newsNum).map((news) => (
                    <News key={news.title} news={news} />
                ))}
                <button onClick={() => setNewsNum(newsNum + 3)} className="text-blue-300 pl-4 pb-3 hover:text-blue-400" >
                    Show more
                </button>
            </div>
            ...
            <div className="sticky top-16 text-gray-700 space-y-3 bg-gray-100 pt-2 rounded-xl w-[90%] xl:w-[75%]">
                ...
                {randomUser.slice(0, randomUserNum).map((randomUser) => (
                    <div
